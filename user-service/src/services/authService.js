...
// Endpoint para comprobar directamente /health del user-service desde el gateway
app.get('/internal/check-user-service', async (req, res) => {
  const target = process.env.USER_SERVICE_URL;
  if (!target) return res.status(500).json({ success: false, error: 'USER_SERVICE_URL no definido' });
  ...
  try {
    ...
    const options = {
      ...
      path: (u.pathname.replace(/\/$/, '') || '') + '/health',
      method: 'GET',
      timeout: 10000
    };
    ...
    r.on('error', err => {
      console.error('check-user-service error ->', err);
      res.status(502).json({ success: false, target, error: err.message });
    });
    ...
    console.error('check-user-service exception ->', err);
    res.status(500).json({ success: false, error: err.message });
  }
});

// Endpoint para comprobar resolución DNS interna
app.get('/internal/dns-check', async (req, res) => {
  const target = process.env.USER_SERVICE_URL;
  if (!target) return res.status(500).json({ success: false, error: 'USER_SERVICE_URL no definido' });
  ...
  try {
    ...
    res.json({ success: true, host, records });
  } catch (err) {
...
// Ejemplo de ajuste de proxyOptions (si usas http-proxy-middleware)
const proxyOptions = {
  changeOrigin: true,
  followRedirects: true,
  secure: false, // solo para pruebas si hiciera falta (HTTP interno no necesita)
  timeout: 20000,
  proxyTimeout: 20000,
  onError: (err, req, res, target) => {
    console.error(`Proxy error to ${target} for ${req.method} ${req.url}:`, err && err.message ? err.message : err);
    if (!res.headersSent) {
      res.statusCode = 502;
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({ success: false, error: err.message || 'proxy_error', target }));
    }
  }
